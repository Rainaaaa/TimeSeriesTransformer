...
class Encoder(nn.Module):
    def __init__(self, embed_size : int, num_returns : int, inner_dim : int,  heads : int, repeats : int, dropout : float):
        ...
        self.sqlayer = nn.Linear(num_returns, 16) #16 for 1 chara
    ...
    def forward(self, returns : torch.Tensor, return_mask : torch.BoolTensor = None):
        # print("input return shape(line 92):", returns.shape)

        returns = self.sqlayer(returns)
        # print("input sqlayered return shape(line 95):", returns.shape)

        returns = returns.unsqueeze(dim = -1)
        # print("input sqlayered unsqueezed return shape(line 98):", returns.shape)

        return_embeds = self.return_embeds(returns)
        # print("return_embeds shape(line 101):", return_embeds.shape)

        encoded = self.dropout(return_embeds)
        # print("encoded shape(line 104):", encoded.shape)
...
class Decoder(nn.Module):
    def __init__(self, embed_size : int, num_returns : int, inner_dim : int,  heads : int, repeats : int, dropout : int):
        ...
        self.sequence_adjustment = nn.Linear(190, 2)
    ...
    def forward(self, enc_out : torch.Tensor, returns : torch.Tensor, return_mask : torch.BoolTensor = None, company_mask = None):
        ...
        # print("input decoder return shape(line 144):", returns.shape)

        returns = returns.unsqueeze(dim = -1)
        # print("input decoder unsqueeze return shape(line 147):", returns.shape)

        return_encodings = self.return_encoding(returns) #### (batch_dates, num_companies, embed_size)
        # print("input decoder return_encodings shape(line 150):", return_encodings.shape)

        decoder_outs = self.dropout(return_encodings)
        # print("before block: decoder_outs shape(line 153):", decoder_outs.shape)

        for decoder_block in self.Decoder_Blocks:
            decoder_outs = decoder_block(decoder_outs, enc_out, return_mask, company_mask)
        
        # print("after block: decoder_outs shape(line 156):", decoder_outs.shape)

        # adjust the dimension of the decoder_outs (for all year)
        decoder_outs = decoder_outs.permute(0, 2, 1)
        decoder_outs = self.sequence_adjustment(decoder_outs)
        decoder_outs = decoder_outs.permute(0, 2, 1) 
        # print("adjusted decoder_outs shape(line 164):", decoder_outs.shape)

        outputs = self.final_linear_layer(decoder_outs)
        # print("decoder outputs shape(line 167):", outputs.shape)
...
class FinTransformer(nn.Module):
    ...
    def forward(self, decoder_inputs : torch.FloatTensor,
        ...
        # print("encoder inputs:", return_inputs.shape)
